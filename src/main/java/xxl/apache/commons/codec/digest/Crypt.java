...
public class Crypt {
    ...
    public static String crypt(final byte[] keyBytes, final String salt) {
        if (salt == null) {
            return xxl.apache.commons.codec.digest.Sha2Crypt.sha512Crypt(keyBytes);
        } else if (salt.startsWith(xxl.apache.commons.codec.digest.Sha2Crypt.SHA512_PREFIX)) {
            return xxl.apache.commons.codec.digest.Sha2Crypt.sha512Crypt(keyBytes, salt);
        } else if (salt.startsWith(xxl.apache.commons.codec.digest.Sha2Crypt.SHA256_PREFIX)) {
            return Sha2Crypt.sha256Crypt(keyBytes, salt);
        } else if (salt.startsWith(xxl.apache.commons.codec.digest.Md5Crypt.MD5_PREFIX)) {
